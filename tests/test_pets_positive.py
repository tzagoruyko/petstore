...
def test_create_new_pet():
    ...
    with allure.step("Создать нового питомца"):
        create_response = pets.add_a_new_pet(payload)

    with allure.step("Запрос создания отдает 200"):
    ...
    with allure.step("Запросить созданного питомца по id"):
        get_response = pets.get_pet_by_id(pet_id)
    with allure.step("Запрос получения по id отдает 200"):
        assert get_response.status_code == 200, "Статус код должен быть 200"

    body = get_response.json()

    with allure.step("Проверка, что id полученного питомца совпадает с id созданного"):
        assert body["id"] == pet_id, "id не соответсвует запрошенному"
    with allure.step("Проверка, что id категории полученного питомца совпадает с id категорией созданного"):
